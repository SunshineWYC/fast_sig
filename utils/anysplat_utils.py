...
def anySplat(dataset, opt, pipe):
    ...
    sparse_dir = os.path.join(dataset.source_path, "sparse/0")
    cameras_extrinsic_bin = os.path.join(sparse_dir, "images.bin")
    cameras_extrinsic_txt = os.path.join(sparse_dir, "images.txt")
    sfm_pts_bin = os.path.join(sparse_dir, "points3D.bin")
    sfm_pts_txt = os.path.join(sparse_dir, "points3D.txt")

    try:
        cam_extrinsics = read_extrinsics_binary(cameras_extrinsic_bin)
    except Exception:
        cam_extrinsics = read_extrinsics_text(cameras_extrinsic_txt)

    try:
        xyz, rgb, _ = read_points3D_binary(sfm_pts_bin)
    except Exception:
        xyz, rgb, _ = read_points3D_text(sfm_pts_txt)  # points from colmap
    imgs = []
    cams_unsorted = []
    ...
    llffhold = 8
    cam_names_sorted = [cam.image_name for cam in cams]
    if getattr(dataset, "eval", False) and llffhold:
        test_names = set(
            [name for idx, name in enumerate(cam_names_sorted) if idx % llffhold == 0]
        )
        train_names = set(cam_names_sorted) - test_names
        if len(train_names) == 0:
            train_names = set(cam_names_sorted)
    else:
        train_names = set(cam_names_sorted)
    slam_c2ws = []
    for cam in cams:
        ...
        if not os.path.exists(image_path):
            image_path = os.path.join(os.path.dirname(images_dir), "images", img_name)
