...
Train/test camera split:

- Default: all cameras are used for training.
- With `--eval`: a LLFF-style hold-out split is applied (every 8th view after sorting by filename is used for test).

AnySplat initialization:

- Disabled by default.
- Enable with `--use_anysplat_init` (requires AnySplat weights/config in `./anySplat/ckpt`).
